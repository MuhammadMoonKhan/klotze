...
import image from './assets/K_Logo.png'
...
const App = () => {
  ...
  const resizeImage = (imgUrl, maxWidth, maxHeight) => {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        const resizedImgUrl = canvas.toDataURL("image/jpeg", 0.6); // Adjust quality if needed
  ...
  const handleScreenshot = async () => {
    if (areaRef.current) {
      ...
      // Ensure all images are fetched via the proxy
      await proxyImages();
  
      // Take the screenshot using html2canvas
      html2canvas(areaRef.current, {
        useCORS: true,
        scale: 2, // Higher scale improves quality (optional)
        ...
        .then((canvas) => {
          ...
          // Optional: Resize the image to a specific resolution
  ...
  const proxyImages = async () => {
    ...
    for (let img of images) {
      ...
      if (src.startsWith("http")) {
        try {
          const proxiedUrl = await fetchProxiedImage(src);
          img.src = proxiedUrl; // Replace the image source with the proxied URL
        } catch (err) {
          console.error("Failed to fetch proxied image:", src, err);
        }
      }
    }
  };
